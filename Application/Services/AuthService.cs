...
public class AuthService : IAuthService
{
    ...
    public async Task<(bool Success, string Message)> UpdateUserByAdminAsync(int userId, UpdateUserDto dto)
    {
        var user = await _users.GetByIdIntAsync(userId);
        ...
        if (!string.IsNullOrWhiteSpace(dto.Email))
        {
            ...
            if (emailExists) return (false, "البريد الإلكتروني الجديد مستخدم بالفعل");
        ...
        {
            return (false, "لم يتم إرسال أي بيانات لتعديلها");
        }
    ...
    public async Task<(bool Success, string Message)> DeleteUserByAdminAsync(int userId)
    {
        if (userId == 1) return (false, "لا يمكن حذف الحساب الرئيسي للنظام");

        var user = await _users.GetByIdIntAsync(userId);
        if (user == null) return (false, "المستخدم غير موجود");

        var result = await _users.DeleteUserAsync(user);

        if (result)
            return (true, "تم حذف المستخدم وكافة سجلاته المرتبطة نهائياً");
        else
            return (false, "حدث خطأ أثناء الحذف النهائي");
    }
    public async Task<UsersStatisticsDto> GetUsersStatisticsAsync()
    {
        var counts = await _users.GetUsersCountByRoleAsync();

        return new UsersStatisticsDto
        {
            TotalCitizens = counts.TryGetValue("CITIZEN", out int citizenCount) ? citizenCount : 0,
            TotalEmployees = counts.TryGetValue("GOVERNMENTEMPLOYEE", out int employeeCount) ? employeeCount : 0
        };
    }
}

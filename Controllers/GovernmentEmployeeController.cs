...
namespace WebAPI.Controllers
{
    ...
    public class GovernmentEmployeeController : ControllerBase
    {
        ...
        [Authorize(Roles = "GovernmentEmployee")]
        [HttpPost("RequestAdditionalInfo/{complaintId}/{citizenId}")]
        public async Task<IActionResult> RequestAdditionalInfo(
     [FromRoute] int complaintId,
     [FromRoute] int citizenId,
     [FromForm] RequestAdditionalInfoDto dto)  
        {
            ...
            var success = await _service.RequestAdditionalInfoAsync(complaintId, citizenId, dto.Message, employeeId);
            ...
            return Ok(new
            {
                message = "تم إرسال طلب معلومات إضافية إلى المواطن",
                complaintId
            });
        }
